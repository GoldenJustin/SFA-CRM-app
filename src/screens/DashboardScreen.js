...
export default function DashboardScreen({ navigation }) {
    ...
    const pingLocation = async () => {
        try {
            ...
            if (loc) {
                const res = await authFetch('/api/method/sfa_crm.api.force_log_location', 'POST', {
                    latitude: loc.coords.latitude, 
                    longitude: loc.coords.longitude,
                    timestamp: new Date().toISOString().replace('T',' ').substring(0, 19), 
                    activity: 'Active Shift Track'
                });
                
                // LOG THE REAL RESPONSE FROM THE SERVER
                if (res && res.message && res.message.status === 'success') {
                    console.log(`[TRACKER]: Ping saved! ID: ${res.message.name}`);
                } else {
                    console.log("[TRACKER]: ERPNext Rejected Ping:", JSON.stringify(res));
                }
            }
        } catch(e) {
            console.log("[TRACKER]: Failed to capture location for ping.");
        }
    };
    ...
    const loadDashboardData = async () => {
        try {
            ...
            const totalVal = orders.reduce((sum, o) => sum + ((Number(o.total) || 0)), 0);
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.walletCard}>
                ...
                <Text style={styles.walletAmount}>Tsh {(Number(stats.totalValue) || 0).toLocaleString()}</Text>
